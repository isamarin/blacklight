<script lang="ts">
	...
	import { REGION_IP_PRESETS } from '$lib/auth/region';
	...
	<div class="flex gap-8">
		...
		<div class="flex-1 space-y-4">
			<Card>
				...
				<select
					class="glass-control w-full px-3 py-2"
			...
			<Card>
				<label class="block text-white/70 text-sm mb-2">
					{t('settings.streaming.setRegionLabel')}
				</label>
				<select
					class="glass-control w-full px-3 py-2"
					...
					onchange={(e) =>
						setSettings({
							...settings,
							force_region_ip: (e.currentTarget as HTMLSelectElement).value
						})}
				>
					{#each REGION_IP_PRESETS as preset (preset.value)}
