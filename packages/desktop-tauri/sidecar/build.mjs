...
const unstamped = path.join(binariesDir, `${sidecarName}${extension}`)
...
const stamped = path.join(binariesDir, `${sidecarName}-${targetTriple}${extension}`)

rmSync(stamped, { force: true })
renameSync(unstamped, stamped)

console.log(`[sidecar] built ${stamped}`)